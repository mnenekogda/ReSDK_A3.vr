...
// Базовая роль для режима GMMain. Рекомендуется именовать все дочерние роли для этого режима с постфиксом имени режима и префиксом R (Role)
// Пример: RCaveExplorer_GMMain
class(GMMain_BasicRole) extends(BasicRole) // BasicRole - базовая роль, от которой унаследованы все возможные роли.
	...
	// Установите true требуется для тех ролей, без которых режим GMMain не сможет полноценно функционировать.
...
/*
	Определяем роли, унаследованные от GMMain_BasicRole

class(RDebugRole_GMMain) extends(GMMain_BasicRole)
	//change if need
	var(name,"Debug role");
