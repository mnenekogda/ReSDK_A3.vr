...
class(GMMain) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GMMain
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GMMain.
	var(duration,t_asMin(60) * 5); // 30 минут
    
    // Имя карты, которая будет создана в режиме GMMain
	getterconst_func(getMapName,"GryaznoyamskV2");
	...
	// Список ролей (имён классов) для режима GMMain, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
			"GMMain_BasicRole"
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GMMain_BasicRole1" встало 2 игрока и на "GMMain_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GMMain_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GMMain_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GMMain"
	};
	...
	// Вызывается когда режим GMMain выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GMMain
	func(onFinish)
	{
		objParams();
